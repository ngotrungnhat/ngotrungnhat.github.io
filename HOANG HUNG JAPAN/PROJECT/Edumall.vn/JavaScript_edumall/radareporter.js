if(!window.jQuery){
  console.log("not jquery,then add new jquery");
...
var RadaTracking = (function() {
  ...
  function readAdditionalData(data) {
    ...
    var user_id = $('meta[name=rada_uid]');
    if(user_id) {
      user_id = user_id.attr("content");
    } else {
      user_id = $('#rada_uid').val();
...
(function(window) {
  ...
  ScrollTracker.prototype._currentDepth = function() {
    ...
    if (this._context === document.body) {
        var d = this._context.scrollTop || (window.pageYOffset ||
        document.documentElement.scrollTop ||
        document.body.scrollTop || 0);
      // return this._context.scrollTop + this._context.clientHeight;
  ...
  //
  ScrollTracker.prototype._show_info = function(){
    var obj = {};
    obj.context_height = this._contextHeight();
    obj.current_depth = this._currentDepth();
    return obj;
  }
...
var RadarTag = (function() {
  ...
  return {
    onReady: function() {
      ...
      triggerTimeSpent();
      triggerScroll();
      [{"selector":".save-data"},{"selector":".add_course_to_cart"},{"selector":".course-learning-card"},{"selector":".btn-purchase-cart"},{"selector":".bbb"}].forEach(bindClickExtend);
      $(document).on("mouseleave", null, function() { var data = tracker.build_data(); data["type_track"] = "mouseleave"; tracker.push_data(data["storage_key"],data); });
      bindFocus(['#section-popular']);
    }
  }
})();
